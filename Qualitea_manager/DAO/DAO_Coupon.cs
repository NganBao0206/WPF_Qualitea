...
namespace DAO
{
    public class DAO_Coupon
    {
        ...
        public static List<Coupon> GetCurrentCoupons(string keyword, decimal? minPer, decimal? maxPer, int? minPoint, int? maxPoint)
        {
            ...
            List<Coupon> coupons = entities.Coupons.Where(c => (c.Amount - c.CouponRedemptions.Count) > 0).ToList();
            coupons = coupons.Where(c => c.StartDate <= DateTime.Now && (c.EndDate == null || ( c.EndDate != null && c.EndDate >= DateTime.Now))).ToList();
            if (keyword != "")
                coupons = coupons.Where(c => c.Name.ToUpper().Contains(keyword.ToUpper())).ToList();
            if (minPer != null)
                coupons = coupons.Where(c => c.PercentageDiscount >= minPer).ToList();
